...
DiceApp.prototype.add = function () {
        ...
        let dice = new Dice(); // initiera ny instans av Dice
        dice.roll();//Anropar Dice instansens roll metod.
        let li = document.createElement("li"); //Skapa ett li element som ska vara en Dice
        li.classList.add("dice") // lägg till klassen dice till li elementet
        li.classList.add(this.diceClasses[dice.value]); //lägg till korrekt klassnamn baserat på vilket värde tärningen fick
       
        this.ulForContent.appendChild(li); // appenda returnerade <li> element från dice.add och fäst det till ulForContent
...
DiceApp.prototype.remove = function () {
    if (this.diceOnBoardValue !== 0) {//hindrar Remove från att utföras om det inte finns några tärningar på brädet
        ...
        this.ulForContent.removeChild(this.ulForContent.lastChild); // ta bort det sista li elementet från this.ulForContent för att grafiskt visa att den sista tärningen tagits bort
        this.displayScore(); // visa det nya värdet på poängen efter att tärningen tagits bort
        this.audio.play(); // spela upp ljud
    };
},
...
DiceApp.prototype.roll = function () {
    ...
    let graphicDiceArray = Array.from(this.ulForContent.children)//Skapar en array med children från ulForContent, dvs tärningarna på brädet

    for (let i = 0; i < this.diceOnBoard.length; i++) {
        this.diceOnBoard[i].roll()    
        this.diceOnBoardValue += this.diceOnBoard[i].value
        graphicDiceArray[i].className = ""//Tar bort klass från ulForContent child[i]
        graphicDiceArray[i].classList.add("dice") // lägg till dice klass
        graphicDiceArray[i].classList.add(this.diceClasses[this.diceOnBoard[i].value]); // lägg till tärningsida klass.
    }
    this.displayScore(); // visa det nya värdet på poängen efter att tärningen tagits bort
    this.audio.play(); // spela upp ljud
    }
